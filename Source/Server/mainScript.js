...
server2.listen(process.env.PORT || 7999);
//server3.listen(process.env.PORT || 7998);
...
// DEMO
//exec('mv File/demo File/AAA',function(err, result){
//	console.log(result.toString());
//});

io2.sockets.on('connection', function (socket2){
	console.log("client connected server 2");
	socket2.on('nameRoad',function(nameRoad){
		console.log(nameRoad);
                conn.query("select * from points where idroad = (select idroad from road where nameroad like ?)",[nameRoad],function(error,results,fields){
                        if(!!error){
                                 console.log(error);
                        }
                        //console.log(results);
			socket2.emit("onGetPointSuccess", {dataResults:results});
			console.log("sent data to client road "+nameRoad);
                 });
	});
});
...
io.sockets.on('connection', function (socket) {
	console.log("admin app  connected server");
	...
	socket.on('username', function (name) {
		...
		conn.query("select adminpass from demo.admintable where adminuser like ?",[socket.name],function(error,results,fields){
                	if(!!error){
                       		 console.log(error);
               		}
       			console.log("checking password . . .")
			//if check pass okie
                	if(pass == results[0].adminpass){
				 console.log("correct password");
                        	 socket.emit("loginResults", {result:"true"});
                	}
                	else{
                        	 socket.emit("loginResults", {result:"false"});
